...
```{r data manipulation and package download, echo=FALSE,include=FALSE}
library(dplyr)
library(ggplot2)
library(maps)
library(sf)
...
**Question 1 Graph 1 State Level**

```{r , include=FALSE}
library(tigris)
```

```{r Question1Graph1,echo=FALSE,message=FALSE,warning=FALSE}
ggplot(data = merged_data) +
  geom_sf(aes(fill = Total_Damage), color = "WHITE", size = 0.25) + 
  scale_fill_viridis_c(option = "magma", direction = -1) +
  labs(title = "Total Monetary Damage by State",
       fill = "Total Damage") +
  geom_sf_text(aes(label = ID), size = 1.5, check_overlap = TRUE, color = "black") +
  theme_void() +
...
```{r Question1 Graph 2, include=FALSE,echo=FALSE,message=FALSE,warning=FALSE}
...
**Question 1 Graph 3 Density Map of Death Caused by Storms**
...
```{r Question 1 Graph 3, echo=FALSE,message=FALSE,warning=FALSE}
library(ggplot2)
...
I believe the density map and the state-level choropleth map are both better than the county level map because of data are too spread and detailed in terms of county's visualization.
Personally, I like the density map better because I had calcuate this density based on a proportion of the individual state total death over the total death across nation. It also highlights the important area like Arizona. 
